...
if not os.path.exists(driver):
    raise Exception("Please update your chrome driver address")


urlhead = "https://zh.m.wikipedia.org"

with open('npc13.csv','w+',encoding='utf-8', newline='') as file:
    writer =csv.writer(file)
    #writer.writerow(["name", "party", "gender", "ethnic", "birthday", "position", "note", "birthplace", "repprov"])      
...
for i in range(6,tablerows_len):
    ...
    indsoup = BeautifulSoup(indreq.read(), features="lxml")
    ...
    output.append("北京市")
    with open('npc13.csv','a',encoding='utf-8', newline='') as file:
        writer =csv.writer(file)
...
csvfile = csv.reader(open('npc13.csv','r',encoding='utf-8'))
npc13info = []
for row in csvfile:
